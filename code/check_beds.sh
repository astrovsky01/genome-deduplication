...
if [[ "$do_check_5" -eq 1 ]]; then
	...
	if [[ "$sample_fa" == *.gz ]]; then
		zcat $sample_fa | awk 'BEGIN {OFS="\t"} /^>/ {if (len) {print name, '0', len}; name=substr($1,2); len=0; next} {len+=length($0)} END {print name, '0', len}' | sortBed >> $chrom_size_bed
	else
		awk 'BEGIN {OFS="\t"} /^>/ {if (len) {print name, '0', len}; name=substr($1,2); len=0; next} {len+=length($0)} END {print name, '0', len}' $sample_fa | sortBed >> $chrom_size_bed
	fi

/**
 * Genome Deduplication Tool - C++ Implementation (with min_len fix)
 * 
...
/**
 * Condense ambiguous positions into regions
 */
std::vector<Region> condense_ambiguous_regions(const std::vector<size_t>& ambiguous) {
    std::vector<Region> ambiguous_regions;
    if (ambiguous.empty()) {
        return ambiguous_regions;
    }
    
    size_t region_start = ambiguous[0];
    for (size_t i = 0; i < ambiguous.size() - 1; ++i) {
        if (ambiguous[i] + 1 != ambiguous[i + 1]) {
            ambiguous_regions.emplace_back(region_start, ambiguous[i] + 1);
            region_start = ambiguous[i + 1];
        }
    }
    ambiguous_regions.emplace_back(region_start, ambiguous.back() + 1);
    return ambiguous_regions;
}
    ...
    if (N_policy == "allow_none") {
        ...
        if (N_index != std::string::npos) {
            if (N_index < static_cast<size_t>(min_sample_len)) {
                ...
                result.ambiguous_idx = N_index;  // Record the N
                result.next_start_offset = N_index + 1;  // Start next sample after the N
            }
        }
    }
        ...
        for (size_t kmer_start_idx = 0; kmer_start_idx <= sample_end_coord - k; ++kmer_start_idx) {
            ...
            if (seen_kmers.find(kmer_num) == seen_kmers.end() &&
                ...
                if (kmer_start_idx < static_cast<size_t>(min_sample_len)) {
                    ...
                    result.duplicate_start_idx = kmer_start_idx;
                    result.next_start_offset = kmer_start_idx + 1;
                    if (kmer_start_idx >= static_cast<size_t>(k - 1)) {
                        result.ignored_region = {static_cast<int>(kmer_start_idx - k + 1), static_cast<int>(kmer_start_idx)};
                    }
...
DeduplicationResult deduplicate_seq(
    ...
    while (sample_start <= max_start_idx) {
        ...
        if (check_result.checked_sample_len > -1) {
            size_t region_end;
            // Check if we stopped early due to duplicate or N
            if (check_result.duplicate_start_idx > -1 || check_result.ambiguous_idx > -1) {
                // Stopped early, extend to cover last checked kmer
                region_end = sample_start + check_result.checked_sample_len + args.kmer - 1;
            } else {
                // Completed full sample normally
                region_end = sample_start + check_result.checked_sample_len;
            }
            result.sample_regions.emplace_back(sample_start, region_end);
        }
    ...
    result.ambiguous_regions = condense_ambiguous_regions(ambiguous_positions);

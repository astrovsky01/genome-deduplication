    ...
    if N_policy == "allow_none":
        ...
        if N_index > -1:
            ...
            if N_index < min_sample_len:
                ...
                # ADD THESE TWO LINES:
                ambiguous_idx = N_index  # Record the N
                next_start_offset = N_index + 1  # Start next sample after the N
        ...
        for kmer_start_idx in range(sample_end_coord-k+1):
            ...
            # If we've seen this kmer before, stop analyzing this sample
                ...
                # Whether the repeat is internal or global, the current kmer is the offending one
                    ...
                    duplicate_start_idx = kmer_start_idx
                    next_start_offset = kmer_start_idx + 1
                    if kmer_start_idx >= k - 1:
                        ignored_region = (kmer_start_idx - k + 1, kmer_start_idx)
...
def deduplicate_seq(seq, seen_kmers, args):
    ...
    while sample_start <= max_start_idx:
        ...
        if checked_sample_len > -1:
            ...
            if duplicate_start_idx > -1 or ambiguous_idx > -1:
                # Stopped early, extend to cover last checked kmer
                region_end = sample_start + checked_sample_len + k - 1
            else:
                # Completed full sample normally
                region_end = sample_start + checked_sample_len
            sample_regions.append((sample_start, region_end))

...
if [ -e ${in_dir}/all_sequences.txt ]; then
    rm ${in_dir}/all_sequences.txt
fi
touch ${in_dir}/all_sequences.txt
...
for file in $ignored_files; do
    ...
    if [[ "$fasta_file" == *.gz ]]; then
        ...
        echo "Retrieving sequences for $file_basename"
        ...
        samtools faidx --fai-idx "${in_dir}/${file_basename}.fai" "$temp_fasta"
        rm "$temp_fasta"
    else
        echo "Retrieving sequences for $file_basename"
        bedtools getfasta -fi "$fasta_file" -bed "$file" -name >> $ignored_sequences
        bedtools getfasta -fi "$fasta_file" -bed "$mask_file" -name >> $masked_sequences
        samtools faidx --fai-idx "${in_dir}/${file_basename}.fai" "$fasta_file"
    fi
done
...
if [ -e ${in_dir}/extracted_ignored_dedup_files.txt ]; then
    rm ${in_dir}/extracted_ignored_dedup_files.txt
fi
if [ -e ${in_dir}/all_extracted_files.txt ]; then
    rm ${in_dir}/all_extracted_files.txt
fi
...
kmc -k${kmer_size} -cs4294967295 -fm -b -ci1 -t${threads} -m${mem} ${in_dir}/all_sequences.txt ${in_dir}/deduped_fasta_files ${in_dir}/kmc_dedup_temp >${in_dir}/deduped_kmer_counts.log
kmc_tools transform ${in_dir}/deduped_fasta_files dump ${output_deduped_file}

echo "Running kmc for ignored"
kmc -k${kmer_size} -cs4294967295 -fm -b -ci1 -t${threads} -m${mem} ${ignored_sequences} ${in_dir}/ignored_fasta_files ${in_dir}/kmc_ignored_temp >${in_dir}/ignored_kmer_counts.log
kmc_tools transform ${in_dir}/ignored_fasta_files dump ${output_ignored_file}

echo "Running kmc for masked"
kmc -k${kmer_size} -cs4294967295 -fm -b -ci1 -t${threads} -m${mem} ${masked_sequences} ${in_dir}/masked_fasta_files ${in_dir}/kmc_masked_temp >${in_dir}/masked_kmer_counts.log
kmc_tools transform ${in_dir}/masked_fasta_files dump ${output_masked_file}

echo "Running kmc for ignored + deduped"
kmc -k${kmer_size} -cs4294967295 -fm -b -ci1 -t${threads} -m${mem} @${in_dir}/extracted_ignored_dedup_files.txt ${in_dir}/ignored_dedup_files ${in_dir}/kmc_ignored_deduped_temp >${in_dir}/ignored_deduped_kmer_counts.log
